...
//1. 변수 정리 : /텍스트 변수, 파일 변수, /이미지 파일 정보 변수, /날짜 변수, /이미지 확장자 필터링 변수, /파일 사이즈 제한 변수
...
$image_tyoe = $_FILES['img_main']['type'];
...
if ($image_name && !$image_error) {
 ...
 if (!move_uploaded_file($image_tmp_name, $upload_file)) {
  echo json_encode(array("msg1" => "사진이 업로드 되지 않습니다."));
  exit();
 }
} else {
  $image_name = "";
...
if (!$stmt->prepare($sql)) {
  ...
  echo json_encode(array("msg3" => "Database insert fail."));
  exit();
} 
